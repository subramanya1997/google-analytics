...
class AnalyticsPostgresClient:
    ...
    def get_locations(self, tenant_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            with self.get_db_session() as session:
                # Get locations that have page view activity
                locations_with_activity = session.execute(text("""
                    SELECT DISTINCT l.warehouse_code, l.warehouse_name, l.city, l.state
                    FROM locations l
                    INNER JOIN page_view pv ON pv.user_prop_default_branch_id = l.warehouse_code
                    WHERE l.tenant_id = :tenant_id AND l.is_active = true
        ...
        try:
            with self.get_db_session() as session:
                ...
                if start_date and end_date:
                    ...
                    filters.append(Purchase.event_date >= datetime.strptime(start_formatted, '%Y%m%d').date())
                    filters.append(Purchase.event_date <= datetime.strptime(end_formatted, '%Y%m%d').date())

                # Purchase data
                purchases = session.query(Purchase).filter(and_(*filters)).all()
                total_revenue = sum(float(p.ecommerce_purchase_revenue or 0) for p in purchases)
                total_purchases = len(purchases)
                purchase_sessions = set(p.param_ga_session_id for p in purchases if p.param_ga_session_id)

                # Cart data with same filters
                cart_filters = [AddToCart.tenant_id == tenant_id]
                if location_id:
                    cart_filters.append(AddToCart.user_prop_default_branch_id == location_id)
                if start_date and end_date:
                    cart_filters.append(AddToCart.event_date >= datetime.strptime(start_formatted, '%Y%m%d').date())
                    cart_filters.append(AddToCart.event_date <= datetime.strptime(end_formatted, '%Y%m%d').date())

                carts = session.query(AddToCart).filter(and_(*cart_filters)).all()
                cart_sessions = set(c.param_ga_session_id for c in carts if c.param_ga_session_id)
                abandoned_carts = len(cart_sessions - purchase_sessions)
                
                # Failed searches
                search_filters = [NoSearchResults.tenant_id == tenant_id]
                if location_id:
                    search_filters.append(NoSearchResults.user_prop_default_branch_id == location_id)
                if start_date and end_date:
                    search_filters.append(NoSearchResults.event_date >= datetime.strptime(start_formatted, '%Y%m%d').date())
                    search_filters.append(NoSearchResults.event_date <= datetime.strptime(end_formatted, '%Y%m%d').date())

                failed_searches = session.query(NoSearchResults).filter(and_(*search_filters)).count()

                # Page view data for visitors and repeat visits
                pv_filters = [PageView.tenant_id == tenant_id]
                if location_id:
                    pv_filters.append(PageView.user_prop_default_branch_id == location_id)
                if start_date and end_date:
                    pv_filters.append(PageView.event_date >= datetime.strptime(start_formatted, '%Y%m%d').date())
                    pv_filters.append(PageView.event_date <= datetime.strptime(end_formatted, '%Y%m%d').date())

                pageviews = session.query(PageView).filter(and_(*pv_filters)).all()
                
                # Total Visitors (unique sessions)
                total_visitors = len(set(pv.param_ga_session_id for pv in pageviews if pv.param_ga_session_id))
    ...
    def get_task_status(self, tenant_id: str, task_id: str, task_type: str) -> Dict[str, Any]:
        ...
        try:
            with self.get_db_session() as session:
                task = session.query(TaskTracking).filter(
    ...
    def get_purchase_tasks(self, tenant_id: str, page: int, limit: int, 
        ...
        try:
            with self.get_db_session() as session:
                ...
                return result or {'data': [], 'total': 0, 'page': page, 'limit': limit, 'has_more': False}
