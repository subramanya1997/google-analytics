...
from common.models.analytics import Tenants
...
class AuthenticationService:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                try:
                    ...
                    if bigquery_raw_config:
                        ...
                        try:
                            service_account = json.loads(service_account_str) if service_account_str else {}
                            bigquery_config["service_account"] = service_account
                        except json.JSONDecodeError as e:
                            logger.error(f"Failed to parse BigQuery service_account JSON: {e}")
                    ...
                    logger.info(f"📊 Configurations found - PostgreSQL: {'✅' if postgres_config else '❌'}, BigQuery: {'✅' if bigquery_config else '❌'}, SFTP: {'✅' if sftp_config else '❌'}")
                ...
                if not self._upsert_tenant_configurations(account_id, postgres_config, bigquery_config, sftp_config, username):
                    return {
                        "success": False,
    ...
    def _test_postgres_connection(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            logger.info(f"🔍 Testing PostgreSQL connection to {config.get('host')}:{config.get('port')}")
    ...
    def _test_bigquery_config(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            logger.info(f"🔍 Testing BigQuery configuration for project: {config.get('project_id')}")
    ...
    def _upsert_tenant_configurations(self, tenant_id: str, postgres_config: Dict[str, Any], bigquery_config: Dict[str, Any], sftp_config: Dict[str, Any], username: str) -> bool:
        """
        ...
        try:
            ...
            with Session(engine) as session:
                ...
                config_data = {
                    ...
                    "bigquery_credentials": json.dumps(bigquery_config.get("service_account", {})),
