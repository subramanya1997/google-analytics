...
class SFTPClient:
    ...
    async def list_files(self, pattern: str = "*.xlsx") -> list:
        ...
        try:
            loop = asyncio.get_event_loop()
            files = await loop.run_in_executor(None, self._list_files_sync, pattern)
            return files
        except Exception as e:
    ...
    async def download_file(self, remote_filename: str) -> str:
        ...
        try:
            ...
            # Download file
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self.sftp.get, remote_path, temp_path)
    ...
    async def get_latest_locations_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(
                f"Error getting locations data from {self.locations_file}: {e}"
            )
            raise
