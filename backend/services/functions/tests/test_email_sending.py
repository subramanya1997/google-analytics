...
class EmailQueueTester:
    ...
    async def send_email_message(
        self, job_id: str, report_date: date, branch_codes: list[str] | None = None
    ) -> bool:
        """
        ...
        """
        branch_info = (
            f"{len(branch_codes)} branches" if branch_codes else "all branches"
        )
        ...
        try:
            ...
            print("✅ Email job record created in database")
        ...
        try:
            ...
            print("   2. It will generate and send email reports (2-5 minutes)")
            print(
                "   3. Job status will be updated from 'queued' → 'processing' → 'completed'"
            )
            print("   4. Check Azure Functions logs to see progress")
            print("   5. Query 'email_sending_jobs' table for job status")
            print("   6. Check 'email_send_history' for individual email results")
...
Note:
  ...
  - Report date defaults to yesterday if not provided
    ...
    if args.report_date:
        try:
            # Parse date string directly to date object (no timezone needed for date-only)
    ...
    success = asyncio.run(
        tester.send_email_message(
            job_id=job_id, report_date=report_date, branch_codes=branch_codes
        )
    )
