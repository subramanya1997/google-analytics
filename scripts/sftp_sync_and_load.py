#!/usr/bin/env python3
"""
SFTP sync script to fetch the UserReport.xlsx file and run data loading with cleanup.
"""
...
def main():
    parser = argparse.ArgumentParser(description="SFTP sync and data loading script for UserReport.xlsx")
    ...
    # Configuration file
    parser.add_argument("--config", default="configs/sftp_config.json", help="JSON configuration file for SFTP")
    
    # Control flags
    parser.add_argument("--download-only", action="store_true", help="Only download UserReport.xlsx, don't run load_data")
    ...
    if args.config and os.path.exists(args.config):
        ...
        for key, value in config.items():
            ...
            if hasattr(args, arg_attr) and getattr(args, arg_attr) == defaults.get(arg_attr):
    ...
    if not args.load_only:
        ...
        try:
            remote_user_file = args.user_file
            local_user_path = os.path.join(args.data_dir, remote_user_file)
            
            # Delete local file if it exists to ensure fresh download
            if os.path.exists(local_user_path):
                os.remove(local_user_path)
                logger.info(f"Deleted existing local file: {local_user_path}")
            
            logger.info(f"Attempting to download {remote_user_file} from SFTP...")
            if not syncer.download_file(remote_user_file, local_user_path, args.remote_path):
                logger.error(f"Failed to download {remote_user_file}")
                return 1
            logger.info(f"Successfully downloaded {remote_user_file}")
    ...
    if not args.download_only:
        ...
        if not all_json_files:
            logger.error("No .json/.jsonl files found in data directory to process.")
            return 1
